...
function getElementAccessibleNameInternal(element, options) {
  ...
  if (allowsNameFromContent || options.embeddedInLabelledBy !== "none" || options.embeddedInLabel !== "none" || options.embeddedInTextAlternativeElement || options.embeddedInTargetElement === "descendant") {
    ...
    const visit = (node) => {
      var _a;
      if (node.nodeType === 1) {
        const display = ((_a = getComputedStyles(node)) == null ? void 0 : _a.getPropertyValue("display")) || "inline";
    ...
    tokens.push(getPseudoContent(getComputedStyles(element, "::before")));
    for (let child = element.firstChild; child; child = child.nextSibling)
      visit(child);
    if (element.shadowRoot) {
    ...
    for (const owned of getIdRefs(element, element.getAttribute("aria-owns")))
      visit(owned);
    tokens.push(getPseudoContent(getComputedStyles(element, "::after")));
